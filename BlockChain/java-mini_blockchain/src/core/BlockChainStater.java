...
public class BlockChainStater {
	...
	public static void main(String[] args) throws Exception {
		...
		Transaction transaction1 = new Transaction(wallet1, wallet2.getPublicKey(), 1.5, Util.getDate());
		block2.addTransaction(transaction1);
		
		Thread.sleep(1000);
		
		// 지갑2에서 지갑3으로 코인을 전송했다는 의미를 가진 트랜잭션을 생성
		Transaction transaction2 = new Transaction(wallet2, wallet3.getPublicKey(), 3.7, Util.getDate());
		...
		Transaction transaction3 = new Transaction(wallet1, wallet3.getPublicKey(), 2.3, Util.getDate());
		block3.addTransaction(transaction3);
		
		Thread.sleep(1000);
		
		// 지갑2에서 지감3으로 코인을 전송했다는 의미를 가진 트랜잭션을 생성
		Transaction transaction4 = new Transaction(wallet2, wallet3.getPublicKey(), 1.4, Util.getDate());
